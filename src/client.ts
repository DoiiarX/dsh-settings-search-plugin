...
/** One searchable item row inside a section, registered by the owning plugin. */
...
/** A section's searchable surface: tab-level label plus item-level rows. */
...
/** Global registration surface sections publish their searchable entries to. */
export interface SettingsSearchRegistry {
  sections: Map<string, SettingsSearchSection>
  register(sectionId: string, spec: SettingsSearchSection): () => void
}
...
interface CandidateRow {
  ...
  tab: string
  anchored: boolean
  /** Candidate came from the slot ledger (declarative, always complete). */
  ledger?: boolean
}

declare global {
  interface Window {
    __DSH_SETTINGS_SEARCH__?: SettingsSearchRegistry
  }
}

function registry(): SettingsSearchRegistry {
  return (globalThis.__DSH_SETTINGS_SEARCH__ ??= {
    sections: new Map(),
...
/** Discover item-level candidates from a rendered section DOM (non-registering plugins). */
function scanSectionItems(sectionEl: HTMLElement): ScannedItem[] {
  const items: ScannedItem[] = []
  const seen = new Set<string>()
  const push = (title: string, desc?: string, keywords?: string): void => {
  ...
  for (const labelEl of sectionEl.querySelectorAll('label')) {
    ...
    const controls = [...labelEl.querySelectorAll('input,select,textarea')]
      .map(el => (el as HTMLInputElement).placeholder || el.getAttribute('aria-label') || '')
      .filter(Boolean)
      .join(' ')
  ...
  for (const titleEl of sectionEl.querySelectorAll('[class*="title"],[class*="heading"]')) {
    ...
    const controls = [...(container?.querySelectorAll('input,select,textarea,button') ?? [])]
      .map(el => (el as HTMLInputElement).placeholder || el.getAttribute('aria-label') || (el.textContent?.trim() ?? ''))
      .filter(Boolean)
      .join(' ')
    push(title, desc, controls)
  }
...
function installSearch(dialog: HTMLElement, slots?: unknown): (() => void) | undefined {
  if (dialog.dataset.settingsSearchReady === 'true') return
  const nav = dialog.querySelector('nav')
  // The dialog element IS the .panel (SettingsRoot puts role=dialog on
  // css.panel), so querySelector('.panel') never matches it; use the dialog
  // itself as the overlay anchor. Section classes (.content / .options /
  // .navLabel) are CSS Modules hashed names, so locate by structure instead
  // of class: content is nav's next sibling, and the scrolling options area
  // is its last child (header precedes it).
  const panel = dialog
  const list = nav?.querySelector<HTMLElement>('div:has(> button[aria-current])')
  ...
  const rows = (): HTMLButtonElement[] => [...list.querySelectorAll<HTMLButtonElement>(':scope > button')]
  ...
  const scanned = new Map<string, ScannedItem[]>()
  const scanActiveSection = (): void => {
  ...
  const navButtonFor = (sectionId: string, label?: string): HTMLButtonElement | undefined => {
  ...
  const candidateRows = (query: string): CandidateRow[] => {
    ...
    interface LedgerEntryLike { options?: { label?: unknown; id?: string } }
    const ledgerSections = (slots as { entries?: (key: string) => LedgerEntryLike[] } | undefined)?.entries?.('settings.section') ?? []
    const ledgerLabels = new Set<string>()
    for (const entry of ledgerSections) {
      const rawLabel = entry?.options?.label
      const label = typeof rawLabel === 'function' ? (rawLabel as () => unknown)() : rawLabel
      const text = String(label ?? entry?.options?.id ?? '').trim()
      if (!text) continue
      ledgerLabels.add(text)
      if (matches(query, text, entry?.options?.id)) {
        results.push({ kind: 'section', sectionId: text, label: text, tab: text, anchored: false, ledger: true })
      }
    }
    ...
    for (const row of rows()) {
      ...
      if (matches(query, label)) {
        results.push({ kind: 'section', sectionId: label, label, tab: label, anchored: false })
      }
    }
            ...
            setTimeout(() => {
              ...
              if (!(target instanceof HTMLElement)) {
                ...
                target = [...(optionsEl?.querySelectorAll('label') ?? [])]
                  .find(labelEl => normalize(labelEl.querySelector('strong')?.textContent ?? '') === normalize(result.itemId ?? '')) ?? null
              }
...
/**
 * Mount the settings search UI: one search field per Settings dialog, with an
 * independent candidate list built from the slot ledger, the optional global
 * registration surface, and rendered-section scanning.
 * @param ctx - client plugin context carrying the injected slots service.
 */
export function apply(ctx: Context): void {
  ...
  ctx.effect(() => {
    ...
    const sync = (): void => {
      const dialogs = new Set([...document.querySelectorAll<HTMLElement>("[role='dialog'][aria-modal='true']")])
